...
*3.1. Анализ базовых требований* [1*, Гл. 8-9]
...
<div style="text-align:center">
<img src="Images/Рисунок%201.5%20Наиболее%20экономичный%20уровень%20производительности.bmp"
...
К документированным требованиям к программному обеспечению должны применяться те же методы управления конфигурацией, что и к другим конечным результатам процессов жизненного цикла программного обеспечения. (Подробное обсуждение приведено в разделе "Управление конфигурацией"). Кроме того, при необходимости индивидуальные требования также могут быть изменены с помощью управления конфигурацией, которое обычно поддерживается инструментом управления требованиями. (см. раздел 8, Инструменты для определения требований к программному обеспечению).

Существует несколько общих категорий методов определения требований, каждый из которых рассмотрен ниже. В спецификации требований для данного проекта также могут использоваться различные методы. Стандарты ISO/IEC/IEEE 29148 [26], а также [1*, Гл. 10-14],
[5, Гл. 4], [6, Гл. 16] и многие другие предлагают шаблоны для документации по требованиям.

*4.1. Спецификация требований к неструктурированному естественному языку* [1*, Гл. 11] [2*, s4.4.1]
...
Другим примером является шаблон спецификации варианта использования, как показано на рисунке 1.6. (Рекомендации по написанию хороших спецификаций вариантов использования приведены в [11]).

<div style="text-align:center">
<img src="Images/Рисунок%201.6%20Пример%20структурированной%20спецификации%20естественного%20языка%20для%20одного%20варианта%20использования.bmp"
alt="Рисунок 1.6. Пример структурированной спецификации естественного языка для одного варианта использования.">
<br>Рисунок 1.6. Пример структурированной спецификации естественного языка для одного варианта использования.
</div>

Другими примерами являются формат пользовательской истории “Как <пользователь> Я хочу
<возможности>, чтобы <получить выгоду>”, а также таблицы решений. (Смотрите также [5, Гл. 4] [6, Гл. 12, Гл. 16] [7, Гл. 2-5].)

*4.3. Спецификация требований, основанная на критериях приемлемости*
...
ATDD [2*, s3.2.3, s8.2] является частью более широкого подхода к разработке на основе тестирования (TDD). (см. раздел "Тестирование программного обеспечения"). Основная идея TDD заключается в том, что тестовые примеры предшествуют созданию. Таким образом, новый производственный код не пишется и существующий код не модифицируется до тех пор, пока хотя бы один тестовый пример не завершится неудачей либо на уровне модульного тестирования, либо на уровне приемочного тестирования. Процесс ATDD состоит из трех этапов:
...
Подход BDD [19] несколько более структурирован, и эксперты в бизнес-областях обычно предпочитают его ATDD, поскольку он менее техничен на вид. В BDD функциональная единица описывается как история пользователя в форме, подобной этой: “В качестве <роли> я хочу
...
Спецификация требований, основанная на критериях приемлемости, напрямую решает проблему неоднозначности требований. Естественные языки по своей сути неоднозначны, но язык тестовых примеров - нет. В спецификации требований, основанной на критериях приемлемости, требования написаны с использованием языка тестовых примеров, который является очень точным. С другой стороны, это по своей сути не решает проблему неполноты. Однако сочетание ATDD или BDD с соответствующими критериями охвата функциональным тестированием, такими как тестирование предметной области, анализ граничных значений и попарное тестирование (см. раздел "Тестирование программного обеспечения"), может снизить вероятность неполноты требований. (см. также [9, c1, c12]).

*4.4. Спецификация требований, основанная на модели* [1*, Гл. 12] [2*, Гл. 5] [4*]

Другим подходом к устранению двусмысленности, присущей естественным языкам, является использование языков моделирования, таких как отдельные элементы Unified Modeling Language ™ (UML) или Systems Modeling Language ™ (SysML). Подобно чертежам, используемым при строительстве зданий, эти языки моделирования могут использоваться без использования компьютерных технологий для точного и лаконичного определения функциональных требований [9, Гл. 1-2]. Эта тема тесно связана с моделями и методами разработки программного обеспечения KA. Модели требований делятся на две основные категории:

1. Структурные модели для определения политик, которые необходимо применять: это модели логических классов, описанные, например, в [9, Гл. 8]. Их также называют концептуальными моделями данных, логическими моделями данных и диаграммами сущностей-отношений.
2. Поведенческие модели для определения процессов, которые должны выполняться: Эти модели включают моделирование вариантов использования, как описано в [9, Гл. 7], диаграммы взаимодействия, как описано в [9, Гл. 9], и моделирование состояний, как описано в [9, Гл. 10]. Другими примерами являются диаграммы действий на языке UML и моделирование потоков данных, как описано в [1*, Гл. 12-13], [8], [10] и [18].

Спецификации требований, основанные на модели, различаются по степени формализации модели. Учесть следующее:

- Гибкое моделирование (см., например, [10]) является наименее формальным. Гибкие модели могут быть не более чем грубыми набросками, целью которых является передача важной информации, а не демонстрация правильного использования обозначений при моделировании. В этом типе моделирования эффект от коммуникации считается более важным, чем форма коммуникации.
- Полуформальное моделирование, например [9, Гл. 6-12], содержит определение семантики языка моделирования ([9, Приложение L]), но формально не доказано, что это определение является полным и непротиворечивым.
- Формальное моделирование, например, Z, венский метод разработки (VDM), язык спецификаций и описаний (SDL) и [5, Гл. 6] имеют очень точно определенную семантику, которая позволяет механически анализировать спецификации на наличие или отсутствие определенных свойств, что помогает избежать критических ошибок в рассуждениях. В этом контексте для разработки использовался термин "корректность по построению". (Смотрите раздел "Формальные методы" в разделе "Модели и методы разработки программного обеспечения").
...
- приоритет (см. раздел "Приоритезация требований" далее в этом КА);
- стабильность (см. раздел "Стабильность и изменчивость требований" далее в этом КА);
- является ли требование общим или является вариантом для разработки семейства продуктов (например, [20]);
- вспомогательные материалы;
- история изменений требования.

Gilb's Planguage (сокращенно от Planning Language) [7] рекомендует такие атрибуты, как масштаб, измеритель, минимум, цель, невыполненные задачи, прошлое, тенденция и рекорд.

*4.6. Дополнительные и всеобъемлющие требования
Спецификация*
...
### 5. Проверка требований (Requirements Validation) [1*, Гл. 17] [2*, s4.5]
...
Обычно используются три метода проверки требований: анализ требований, моделирование и выполнение, а также создание прототипов. (см. также [5, Гл. 5] [6, Гл. 17] [9, Гл. 12].)

*5.1. Проверка требований* [1*, Гл. 17 стр. 332-342] [2*, Гл. 4 стр. 130]
...
*6.2. Управление изменениями требований* [1*, Гл. 28] [2*, s4.6]

Контроль изменений занимает центральное место в управлении требованиями. Этот раздел тесно связан с разделом "Управление конфигурацией программного обеспечения". (Обратитесь к этой главе для получения дополнительной информации).
...
<div style="text-align:center">
<img src="Images/Формула.bmp"
alt="формула"
width="300">
<br>
</div>

это всего лишь один из примеров целевой функции, позволяющей это сделать. Выбор схем измерения соответствующих факторов может накладывать ограничения на целевую функцию. (см. Теорию измерений в разделе Основы вычислительной техники).
...
*7.3. Отслеживание требований* [1*, Гл. 29]
...
### 8. Инструменты для определения требований к программному обеспечению (Software Requirements Tools) [1*, c30]
...
*8.3. Инструменты для создания функциональных тестовых наборов*

Чем более формально определен язык спецификации требований, тем больше вероятность того, что функциональные тестовые сценарии могут быть, по крайней мере, частично получены механическим путем. Например, преобразование сценариев BDD в тестовые сценарии не представляет сложности. Другой пример связан с моделями состояний. Положительные тестовые случаи могут быть получены для каждого определенного перехода в модели такого типа. Отрицательные тестовые случаи могут быть получены из комбинаций состояний и событий, которые не отображаются. (Дополнительную информацию смотрите в разделе 8.2 "Инструменты тестирования" в руководстве по тестированию.) Процесс получения тестовых примеров из моделей требований UML можно найти в [9, c12].
...
## МАТРИЦА ТЕМ В СРАВНЕНИИ СО СПРАВОЧНЫЙ МАТЕРИАЛ (MATRIX OF TOPICS VS. REFERENCE MATERIAL)

|                        |               Wiegers 2013 [1*]                   |                   Sommerville 2018 [2*]               |              Tockey 2005 [3*]               |               Wing 1990 [4*]     |
|------------------------|---------------------------------------------------|-------------------------------------------------------|---------------------------------------------|----------------------------------|
| **1. Требования к программному обеспечению Основные положения <br>(Software Requirements Fundamentals)**                                                      |                     |                |  |           |
| 1.1. Определение программного обеспечения Требование <br>(Definition of a Software Requirement)                                                               | Гл. 1 стр. 5-6      | Гл. 4 стр. 102 |  |           |
| 1.2. Категории программного обеспечения Требования <br>(Categories of Software Requirements)                                                                  | Гл. 1 стр. 7-12     | s4.1           |  |           |
| 1.3. Требования к программному продукту и Требования к проекту программного обеспечения <br>(Software Product Requirements and Software Project Requirements) | Гл. 1 стр. 14-15    |                |  |           |
| 1.4. Функциональные требования <br>(Functional Requirements)                                                                                                  | Гл. 1 стр. 9        | s4.1.1         |  |           |
| 1.5. Нефункциональные требования <br>(Nonfunctional Requirements)                                                                                             | Гл. 1 стр. 10-11    | s4.1.2         |  |           |
| 1.6. Технологические ограничения <br>(Technology Constraints)                                                                                                 |                     |                |  |           |
| 1.7. Ограничения, связанные с качеством обслуживания <br>(Quality of Service Constraints)                                                                     |                     |                |  |           |
| 1.8. Зачем Классифицировать Требования таким образом Способ? <br>(Why Categorize Requirements This Way?)                                                      |                     |                |  |           |
| 1.9. Системные требования и программное обеспечение Требования <br>(System Requirements and Software Requirements)                                            |                     |                |  |           |
| 1.10. Производные требования <br>(Derived Requirements)                                                                                                       |                     |                |  |           |
| 1.11. Действия, связанные с требованиями к программному обеспечению <br>(Software Requirements Activities)                                                    | Гл. 1 стр. 15-18    | s4.2           |  |           |
| **2. Выявление требований <br>(Requirements Elicitation)**                                                                                                    |                     |                |  |           |
| 2.1. Источники требований <br>(Requirements Sources)                                                                                                          | Гл. 6               | s4.3           |  |           |
| 2.2. Выявление общих требований Методы <br>(Common Requirements Elicitation Techniques)                                                                       | Гл. 7               | s4.3           |  |           |
| **3. Анализ требований <br>(Requirements Analysis)**                                                                                                          |                     |                |  |           |
| 3.1. Анализ основных требований <br>(Basic Requirements Analysis)                                                                                             | Гл. 8-9             |                |  |           |
| 3.2. Экономика качества обслуживания Ограничения <br>(Economics of Quality of Service Constraints)                                                            |                     |                | Гл. 1-27 |   |
| 3.3. Формальный анализ <br>(Formal Analysis)                                                                                                                  |                     | s12.3.2-12.3.3 |  |           |
| 3.4. Разрешение конфликта в Требования <br>(Addressing Conflict in Requirements)                                                                              |                     |                |  |           |
| **4. Спецификация требований <br>(Requirements Specification)**                                                                                               |                     |                |  |           |
| 4.1. Неструктурированный естественный язык Спецификация требований <br>(Unstructured Natural Language Requirements Specification)                             | Гл. 11              | s4.4.1         |  |           |
| 4.2. Структурированный естественный язык Спецификация требований <br>(Structured Natural Language Requirements Specification)                                 | Гл. 8               | s4.4.2         |  |           |
| 4.3. Требования, основанные на критериях приемлемости Спецификация требований <br>(Acceptance Criteria-Based Requirements Specification)                      |                     | s3.2.3, s8.2   |  |           |
| 4.4. Требования, основанные на модели Спецификация <br>(Model-Based Requirements Specification)                                                               | Гл. 12              | Гл. 5          |  | Стр. 8-11 |
| 4.5. Дополнительные атрибуты требований <br>(Additional Attributes of Requirements)                                                                           | Гл. 27 стр. 462-463 |                |  |           |
| 4.6. Поэтапный и всеобъемлющий подход Спецификация требований <br>(Incremental and Comprehensive Requirements Specification)                                  |                     |                |  |           |
| **5. Проверка требований <br>(equirements Validation)**                                                                                                       |                     |                |  |           |
| 5.1. Анализ требований <br>(Requirements Reviews)                                                                                                             | Гл. 17 стр. 332-342 | Гл. 4 стр. 130 |  |           |
| 5.2. Моделирование и выполнение <br>(Simulation and Execution)                                                                                                |                     |                |  |           |
| 5.3. Создание прототипов <br>(Prototyping)                                                                                                                    | Гл. 17 стр. 342     | Гл. 4 стр. 130 |  |           |
| 6. Управление требованиями Мероприятия <br>(Requirements Management Activities)                                                                               |                     |                |  |           |
| 6.1. Уточнение требований <br>(Requirements Scrubbing)                                                                                                        |                     |                |  |           |
| 6.2. Контроль изменений требований <br>(Requirements Change Control)                                                                                          | Гл. 28              | s4.6           |  |           | 
| 6.3. Согласование области применения <br>(Scope Matching)                                                                                                     |                     |                |  |           |
| **7. Практические соображения <br>(Practical Considerations)**                                                                                                |                     |                |  |           |
| 7.1. Итеративный характер процесса разработки требований Технологические Требования <br>(Iterative Nature of the Requirements	Process)                        |                     | s4.2           |  |           |
| 7.2. Определение приоритетности требований <br>(Requirements Prioritization)                                                                                  | Гл. 16              |                |  |           |
| 7.3. Отслеживание требований <br>(Requirements Tracing)                                                                                                       | Гл. 29              |                |  |           |
| 7.4. Стабильность требований и Нестабильность <br>(Requirements Stability and Volatility)                                                                     |                     | s4.6           |  |           |
| 7.5. Требования к измерениям <br>(Measuring Requirements)                                                                                                     | Гл. 19              |                |  |           |
| 7.6. Требования к качеству процесса и Улучшение <br>(Requirements Process Quality and Improvement)                                                            | Гл. 31              |                |  |           |
| **8. Инструменты для определения требований к программному обеспечению <br>(Software Requirements Tools)**                                                    |                     |                |  |           |
| 8.1. Инструменты управления требованиями <br>(Requirements Management Tools)                                                                                  | Гл. 30 стр. 506-510 |                |  |           |
| 8.2. Инструменты моделирования требований <br>(Requirements Modeling Tools)                                                                                   | Гл. 30 стр. 506     | s12.3.3        |  |           |
| 8.3. Создание функционального тестового примера Инструменты <br>(Functional Test Case Generation Tools)                                                       |                     |                |  |           |
 
## ЧИТАТЬ ДАЛЕЕ

П. Лапланте, Разработка требований к программному обеспечению и системам [5].

Эта книга является одной из возможных альтернатив [1*] и предлагает всестороннее обсуждение требований к программному обеспечению.

Робертсон, "Процесс разработки требований: правильная формулировка требований" [6].

Эта книга является еще одной потенциальной альтернативой [1*] и предлагает всестороннее обсуждение требований к программному обеспечению.

Т. Гилб, "Конкурентная инженерия: Руководство по системной инженерии, разработке требований и разработке программного обеспечения с использованием Planguage" [7].

В этой книге представлен уникальный взгляд на требования, подчеркивающий точность и полноту требований наряду с сильной мотивацией, основанной на ценности бизнеса.

К. Вигерс, "Жемчужины разработки программного обеспечения: уроки пятидесятилетнего опыта разработки программного обеспечения" [21].

Эта книга представляет собой сборник важных, но часто не осознаваемых ключевых факторов успеха, основанных на

Требования к программному обеспечению 1-23

Обширный опыт доктора Вигерса в реальной жизни. В главе 2 конкретно рассматриваются требования к программному обеспечению.

Р. Фишер и У. Юри пришли к выводу, что да [25].

Эта книга представляет собой классический справочник по принципиальным переговорам и разрешению конфликтов, который служит хорошей основой для решения неизбежных конфликтов в требованиях к программному обеспечению при наличии множества заинтересованных сторон.

Н. Ахмад, Влияние электронной коммуникации на выявление неявных знаний в методах проведения интервью для небольших разработок программного обеспечения [29].

В этой докторской диссертации показано, как использование четырех различных типов электронных средств коммуникации для обсуждения деталей повестки дня собеседования с интервьюируемыми перед проведением полуструктурированных интервью для выявления требований улучшает выявление неявных (скрытых) знаний.

## Ссылка на литературу

[1*] К. Э. Вигерс и Дж. Битти, Требования к программному обеспечению, 3-е изд., Редмонд, Вашингтон: Microsoft Press, 2013.

[2*] И. Соммервилл, Разработка программного обеспечения, 10-е изд., Нью-Йорк: Addison-Wesley, 2018.

[3*] С. Токей, Рентабельность программного обеспечения: Максимизация отдачи от инвестиций в программное обеспечение, Бостон, Массачусетс: Addison-Wesley, 2005.

[4*] Дж. М. Винг, “Введение спецификатора в формальные методы”, Компьютер, том 23, № 9, 1990, стр. 8, 10-23.

[5] С. ЛаПланте, Разработка требований к программному обеспечению и системам, 3-е изд., Бока-Ратон, Флорида: CRC Press, 2018.

[6] С. Робертсон и Дж. Robertson, "Освоение процесса разработки требований: правильное выполнение требований", Аппер-Сэддл-Ривер, Нью-Джерси: Addison-Wesley, 2013.

[7] Т. Гилб, Конкурентная инженерия: Руководство по системной инженерии, разработке требований и разработке программного обеспечения с использованием Planguage, Оксфорд, Великобритания: Elsevier Butterworth-Heinemann, 2005.

[8] Э. Вашдон, "Современный структурированный анализ", Энглвуд Клиффс, Нью-Джерси: Прентис-Холл, 1989.

[9] С. Токей, "Как разрабатывать программное обеспечение", Хобокен, Нью-Джерси: Уайли, 2019.

[10] С. Эмблер, "Гибкое моделирование: эффективные методы экстремального программирования и унифицированный процесс", Хобокен, Нью-Джерси: Wiley, 2002.

[11] А. Кокберн, "Создание эффективных сценариев использования", Аппер-Сэддл-Ривер, Нью-Джерси: Эддисон-Уэсли, 2000.

[12] Л. Константин и Л. Локвуд, "Программное обеспечение для использования", Рединг, Массачусетс: Эддисон-Уэсли, 2000.

[13] Дж. Вуд и Д. Сильвер, Совместная разработка приложений, Нью-Йорк, Нью-Йорк: Wiley, 1995.

[14] Э. Готтесдинер, "Требования к сотрудничеству", Бостон, Массачусетс: Addison- Wesley, 2002.

[15] Дж. Тернинко, Пошаговая инструкция, 2-е изд., Бока-Ратон, Флорида: CRC Press, 1997.

[16] Дж. Сальвенди, Справочник по человеческим факторам, 4-е изд., Хобокен, Нью-Джерси: Wiley, 2012.

[17] Т. Браун и Б. Кац, "Изменения с помощью дизайна: как дизайн-мышление трансформирует организации и вдохновляет на инновации", Пересмотренное и обновленное издание, Нью-Йорк, Нью-Йорк: Harper Collins, 2019.

[18] С. Макменамин и Дж. Палмер, Essential Systems Analysis, Нью-Йорк, Нью-Йорк: Yourdon Press, 1984.

[19] Дж. Smart, BDD в действии: разработка на основе поведения для всего жизненного цикла программного обеспечения, Шелтер-Айленд, Нью-Йорк: Manning Publications, 2015.

[20] Д. Вайс и К. Лай, Разработка линейки программных продуктов: процесс разработки программного обеспечения на базе семейства, Рединг, Массачусетс: Эддисон-Уэсли, 1999.

[21] К. Вигерс, "Жемчужины разработки программного обеспечения: уроки пятидесятилетнего опыта разработки программного обеспечения", Бостон, Массачусетс: Addison-Wesley Professional, 2021.

[22] С. Макконнелл, "Быстрое развитие", Редмонд, Вашингтон: Microsoft Press, 1996.


[23] О. Готель и К. В. Финкельштейн, “Анализ проблемы прослеживаемости требований”, представленный в материалах 1-й Международной конференции по разработке требований, 1994 год.

[24] INCOSE, Systems Engineering Handbook: Руководство по процессам и действиям жизненного цикла системы, 3.2.2 изд., Сан-Диего, США: Международный совет по системной инженерии, 2012.

[25] Р. Фишер и У. Юри, Getting to Yes, 3-е изд., Нью-Йорк, Нью-Йорк: Penguin, 2011.

[26] ISO/IEC/IEEE 29148 “Разработка систем и
программного обеспечения – процессы жизненного цикла".
– Разработка требований”, Международная организация по стандартизации, 2018.

[27] ISO/IEC 25010: “Разработка систем и программного обеспечения – Требования и оценка качества систем и программного обеспечения (SQuaRE) – Модели качества систем и программного обеспечения”, Международная организация по стандартизации, 2011.

[28] Словарь программной и системной инженерии, https://pascal.computer.org/sev_display/ind например, действие.

[29] Н. Ахмад, Влияние электронной коммуникации на выявление скрытых знаний в методах проведения собеседований при разработке небольших программных продуктов, докторская диссертация, Университет Хаддерсфилда, 2021 год.